...
# Prepare features for the live data
def prepare_live_features(xbi_data, spy_data):
    """Prepare features for the live XBI data, including SPY-related features."""
    # Calculate indicators for XBI
    xbi_data['7d_ma_XBI'] = xbi_data['close'].rolling(window=7).mean()
...
def load_model():
    ...
    model_path = 'models/xbi_decision_tree_model_latest.pkl'
    with open(model_path, 'rb') as f:
        model = pickle.load(f)
    return model

# Prediction function using the trained model
def predict_market_condition(model, live_features):
    """Predict market condition using the trained model and live features."""
    # Convert the live features into a DataFrame
    live_features_df = pd.DataFrame([live_features])
    
    # Predict the market condition (Top, Neutral, or Bottom)
    prediction = model.predict(live_features_df)
    return prediction[0]

# Streamlit application
def main():
    """Streamlit application to predict market conditions."""
    ...
    # Prepare features
    live_features = prepare_live_features(xbi_data, spy_data)

    # Make prediction
    prediction = predict_market_condition(model, live_features)

    # Display results
    if prediction == 1:
        st.markdown("### Prediction: **Top**")
        st.write("The market is predicted to be in a **Top** condition.")
    elif prediction == -1:
        st.markdown("### Prediction: **Bottom**")
        st.write("The market is predicted to be in a **Bottom** condition.")
    else:
        st.markdown("### Prediction: **Neutral**")
        st.write("The market is predicted to be in a **Neutral** condition.")

    # Optionally, plot the most recent XBI data and indicators
    st.subheader("Latest XBI Data")
    st.write(xbi_data.tail())

    st.subheader("Latest SPY Data")
    st.write(spy_data.tail())
